    ...
    while (!recordingToDownload) {
        ...
        if (recordingToDownload && recordingToDownload.download_url) {
            try {
                ...
                // Fix the code below to download the recording and submit it to Symbl.ai
                const filePath = `../recordings/${recordingIdToFind}`
                // Submit the downloaded recording to Symbl.ai
                await handleSubmitRecordingToSymbl(filePath);


                console.log(`Recording ${recordingIdToFind} downloaded and submitted to Symbl.ai successfully.`);

                // Break the loop since the recording has been successfully processed
                return res.status(200).send('Recording downloaded and submitted to Symbl.ai.');
            } catch (error) {
                console.error(`Error processing recording ${recordingIdToFind}:`, error);
                return res.status(500).send('Failed to process the recording.');
            }
        }

        // Update the nextPageToken to go to the next page of results
        nextPageToken = recordingDetails.next_page_token;
        if (!nextPageToken) {
            // No more pages left to check
            console.log('No more pages available.');
            break;
        }
    }

    // If the recording wasn't found after all pages were checked
...
// Function to submit the recording to Symbl.ai
const handleSubmitRecordingToSymbl = async (filePath) => {
    try {
        const fileData = fs.createReadStream(filePath);

        // Send the file to Symbl.ai
        const response = await axios.post('https://api.symbl.ai/v1/process/video', fileData, {
            headers: {
                'Authorization': `Bearer ${process.env.SYMBL_JWT_TOKEN}`,
                'Content-Type': 'video/mp4'
            }
        });

        console.log(`Symbl.ai processing started for ${filePath}:`, response.data);
        return response.data;
    } catch (error) {
        console.error('Error uploading recording to Symbl.ai:', error.response?.data || error.message);
        throw new Error('Symbl.ai upload failed');
    }
};
